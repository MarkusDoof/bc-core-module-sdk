...
static void _bc_radio_task(void *param)
{
    ...
    while (bc_queue_get(&_bc_radio.rx_queue, queue_item_buffer, &queue_item_length))
    {
        ...
        else if (queue_item_buffer[0] == BC_RADIO_IRRIGATION_PRIMARY_SWITCH_OFF)
        {
            bc_radio_on_irrigation_primary_switch_off();
        }
        else if (queue_item_buffer[0] == BC_RADIO_IRRIGATION_SECONDARY_SWITCH_ON)
        {
            bc_radio_on_irrigation_secondary_switch_on();
        }
        else if (queue_item_buffer[0] == BC_RADIO_IRRIGATION_SECONDARY_SWITCH_OFF)
        {
